...
public class CachoEquipos extends BaseExample implements
	...
	private BitmapTextureAtlas mjugador;
...
private boolean menuseg=false;
	...
	private TextureRegion tjugador;
	private AnimatedSprite boton3;
	private Music mMusic;
	private long lastShakeTime = 0;
	private float lastShakeX, lastShakeY, lastShakeZ = 0;
	private Sound tic;
	private static final int unoRecibido = Integer.MIN_VALUE;
	private static final String vr[] = new String[5];
	private int conf = 0;
	private BitmapTextureAtlas botones;
	private TextureRegion tguardar;
	private TextureRegion tsobre;
	private Sprite pasar;
	private Sprite sobre;
	...
	public Engine onLoadEngine() {
		...
		return new Engine(new EngineOptions(true, ScreenOrientation.LANDSCAPE,
				new FillResolutionPolicy(), this.camara).setNeedsSound(true)
				.setNeedsMusic(true));
	}
	...
	public void onLoadResources() {
		...
		this.mFontTexture = new BitmapTextureAtlas(256, 256,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		this.botones = new BitmapTextureAtlas(512, 128,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		...
		this.tguardar = BitmapTextureAtlasTextureRegionFactory.createFromAsset(
				this.botones, this, "guardar.png", 0, 0);
		this.tsobre = BitmapTextureAtlasTextureRegionFactory.createFromAsset(
				this.botones, this, "sobre.png", 0, 64);
		...
		this.texturaboton3 = BitmapTextureAtlasTextureRegionFactory
				.createTiledFromAsset(this.mboton3, this, "lanzar.png", 0, 0,
						2, 1);
		...
		try {
			this.mMusic = MusicFactory.createMusicFromAsset(
					this.mEngine.getMusicManager(), this, "Dado2.ogg");
		...
		this.mEngine.getTextureManager().loadTextures(this.mapa, this.mboton1,
				...
				this.mBackgroundTexture2,this.mBackgroundTexture, this.mFontTexture, this.mFontTexture2,
				 this.mmenu, this.mbocina, this.mcacho,
				this.botones,this.maceptar);
	...
	public Scene onLoadScene() {
		...
		viny = true;
		pasar = new Sprite(400, 290, this.tguardar) {
			@Override
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent,
					final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
				if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_DOWN) {
		...
		sobre = new Sprite(400, 290, this.tsobre) {
			@Override
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent,
					final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
				if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_DOWN) {
		...
		escena.attachChild(pasar);
		escena.attachChild(sobre);
		pasar.setScale(1.2f);
		sobre.setScale(1.2f);
		...
		boton1 = new Sprite(250, 380, this.texturaboton1) {
			...
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent,
				...
				if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_DOWN) {
					...
					if (ntiros == 0) {
						CachoEquipos.this.libreta.setPosition(-40, 380);
						puntuacion1.setText("");
						puntuacion2.setText("");
						puntuacion3.setText("");
						puntuacion4.setText("");
						...
						CachoEquipos.this.salirPuntuacion2();
			...
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent,
				...
				if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_DOWN) {
					...
					if (cubilete.collidesWith(dado1)
						...
						if (ntiros == 0) {
							CachoEquipos.this.libreta.setPosition(0, 50);
							CachoEquipos.this.textear();
		...
		boton3 = new AnimatedSprite(30, 30, this.texturaboton3) {
			...
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent,
				...
				if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_DOWN) {
					...
					this.setCurrentTileIndex(0);
					if ((CachoEquipos.this.cubilete.collidesWith(dado1)
							|| CachoEquipos.this.cubilete.collidesWith(dado2)
							|| CachoEquipos.this.cubilete.collidesWith(dado3)
							|| CachoEquipos.this.cubilete.collidesWith(dado4) || CachoEquipos.this.cubilete
								.collidesWith(dado5)) && !swtabla && viny) {
						if (!sw1) {
							swaps = 0;
							swespecial = false;
							if (CachoEquipos.this.cubilete.collidesWith(dado1)
									&& CachoEquipos.this.cubilete
											.collidesWith(dado2)
									&& CachoEquipos.this.cubilete
											.collidesWith(dado3)
									&& CachoEquipos.this.cubilete
							...
							CachoEquipos.this.mEngine.vibrate(350);
							if (sound) {
								CachoEquipos.this.musica.play();
							}
							if (CachoEquipos.this.dado1.collidesWith(cubilete)) {
								CachoEquipos.this.dado1.animate(20);
								CachoEquipos.this.dado1.setPosition(390, 40);
								CachoEquipos.this.dado1.setVisible(true);
								CachoEquipos.this.dado1.setScale(1.5f);
							}
							if (CachoEquipos.this.dado2.collidesWith(cubilete)) {
								CachoEquipos.this.dado2.animate(20);
								CachoEquipos.this.dado2.setPosition(590, 40);
								CachoEquipos.this.dado2.setVisible(true);
								CachoEquipos.this.dado2.setScale(1.5f);
							}
							if (CachoEquipos.this.dado3.collidesWith(cubilete)) {
								CachoEquipos.this.dado3.animate(20);
								CachoEquipos.this.dado3.setPosition(490, 40);
								CachoEquipos.this.dado3.setVisible(true);
								CachoEquipos.this.dado3.setScale(1.5f);
							}
							if (CachoEquipos.this.dado4.collidesWith(cubilete)) {
								CachoEquipos.this.dado4.animate(20);
								CachoEquipos.this.dado4.setPosition(540, 190);
								CachoEquipos.this.dado4.setVisible(true);
								CachoEquipos.this.dado4.setScale(1.5f);
							}
							if (CachoEquipos.this.dado5.collidesWith(cubilete)) {
								CachoEquipos.this.dado5.animate(20);
								CachoEquipos.this.dado5.setPosition(410, 190);
								CachoEquipos.this.dado5.setVisible(true);
								CachoEquipos.this.dado5.setScale(1.5f);
							}
							int a = (int) System.currentTimeMillis();
							a = a + 1700;
							while ((int) System.currentTimeMillis() <= a) {
							}
							if (CachoEquipos.this.dado1.isAnimationRunning()) {
								datoDado1 = (int) (Math.random() * 6);
								CachoEquipos.this.dado1.stopAnimation();
								CachoEquipos.this.dado1
										.setCurrentTileIndex(datoDado1);
								escena.registerTouchArea(dado1);
							}
							if (CachoEquipos.this.dado2.isAnimationRunning()) {
								datoDado2 = (int) (Math.random() * 6);
								CachoEquipos.this.dado2.stopAnimation();
								CachoEquipos.this.dado2
										.setCurrentTileIndex(datoDado2);
								escena.registerTouchArea(dado2);
							}
							if (CachoEquipos.this.dado3.isAnimationRunning()) {
								datoDado3 = (int) (Math.random() * 6);
								CachoEquipos.this.dado3.stopAnimation();
								CachoEquipos.this.dado3
										.setCurrentTileIndex(datoDado3);
								escena.registerTouchArea(dado3);
							}
							if (CachoEquipos.this.dado4.isAnimationRunning()) {
								datoDado4 = (int) (Math.random() * 6);
								CachoEquipos.this.dado4.stopAnimation();
								CachoEquipos.this.dado4
										.setCurrentTileIndex(datoDado4);
								escena.registerTouchArea(dado4);
							}
							if (CachoEquipos.this.dado5.isAnimationRunning()) {
								datoDado5 = (int) (Math.random() * 6);
								CachoEquipos.this.dado5.stopAnimation();
								CachoEquipos.this.dado5
										.setCurrentTileIndex(datoDado5);
								escena.registerTouchArea(dado5);
							}
							CachoEquipos.this.mEngine.vibrate(50);
							sw1 = false;
							ntiros++;
							calcularLista();
							if (ntiros >= 2) {
		...
		puntuacion1 = new ChangeableText(80, 120, this.mFont2, "  ", 3) {
			...
			final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
				...
				} else if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_UP && !menuseg) {
					conf=0;
					if (g[np][1] == 0) {
						calcularLista();
						...
						if (g[np][1] == 1
							...
							if (turnos == 0) {
								...
								//turnos++;
								menuf.setPosition(0,0);
								menuf.setVisible(true);
								menuseg=true;
								submenu=new CameraScene(CachoEquipos.this.camara);
								submenu.setBackgroundEnabled(false);
								tA = new Text(300,150,mFont2,"Anotar"){
									@Override
									public boolean onAreaTouched(final TouchEvent pSceneTouchEvent,
									...
									final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
										if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_UP) {
											...
											escena.clearChildScene();
											/*CachoEquipos.this.runOnUiThread(new Runnable() {
												@Override
												public void run() {
													escena.unregisterTouchArea(tA);
													escena.detachChild(tA);
													escena.unregisterTouchArea(tS);
													escena.detachChild(tS);
												}
											});*/
											/*escena.unregisterTouchArea(this);
											escena.detachChild(this);
											escena.unregisterTouchArea(tS);
											escena.detachChild(tS);*/
											menuf.setVisible(false);
											terminar();
								...
								tS = new Text(200,250,mFont2,"Jugar sobre esto"){
									@Override
									public boolean onAreaTouched(final TouchEvent pSceneTouchEvent,
									...
									final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
										if (pSceneTouchEvent.getAction() == MotionEvent.ACTION_UP) {
											...
											menuseg=false;
											anoto = false;
											// CachoEquipos.this.puntuacion1.setText("~~");
											guardado = String
													.valueOf(v[0]);
											flecha = 1;
											turnos++;
								...
								/*escena.registerTouchArea(tA);
								escena.attachChild(tA);
								escena.registerTouchArea(tS);
								escena.attachChild(tS);	*/
								submenu.attachChild(tA);
								submenu.attachChild(tS);
								submenu.registerTouchArea(tA);
								submenu.registerTouchArea(tS);
								escena.setChildScene(submenu, false, true, true);
...
if (turnos == 0) {
		...
		escena.registerUpdateHandler(new IUpdateHandler() {
			...
			public void onUpdate(final float pSecondsElapsed) {
				...
				if (viny == false) {
					Intent a = new Intent(CachoEquipos.this, CachoEquipos.class);
					CachoEquipos.this.startActivity(a);
					CachoEquipos.this.finish();
				}

			}
		});
		menuf = new Sprite(210, 100, this.tmenu);
	...
	public void onAccelerometerChanged(final AccelerometerData ace) {
		...
		if ((this.cubilete.collidesWith(dado1)
			...
			if ((ace.getZ() <= -7 && ace.getY() <= 7 && ace.getX() <= 1)
				...
				if (sound) {
					if (mMusic.isPlaying()) {
						CachoEquipos.this.mMusic.pause();
					}
					CachoEquipos.this.musica.play();
				}
			...
			if ((currentTime - lastShakeTime) > 100) {
				...
				float speed = Math.abs(x + y + z - lastShakeX - lastShakeY
						- lastShakeZ)
						/ diffTime * 10000;
				// Log.i("shake (speed)", String.format("%f", speed));
				if (speed > 1000) // set this lower or higher to change the
									// sensitivity
				{   
					// do something -- we detected a shake
					// mostrar("agitado");
					this.mEngine.vibrate(250);
					if (sound) {
						if (!mMusic.isPlaying()) {
							CachoEquipos.this.mMusic.play();
						}
					}
				}
				lastShakeX = x;
	...
	public void calcularLista() {
		...
		if ((swespecial || ntiros == 1) && swaps == 0) {
			...
			if (v[0] == 5 || v[1] == 5 || v[2] == 5 || v[3] == 5 || v[4] == 5
				...
				this.escena.unregisterTouchArea(bocina);
	...
	public boolean onKeyDown(final int pKeyCode, final KeyEvent pEvent) {
		...
		if (pKeyCode == KeyEvent.KEYCODE_MENU
			...
			if (this.escena.hasChildScene()) {
				...
				menuf.setVisible(true);
				CachoEquipos.this.escena.setChildScene(
						CachoEquipos.this.mMenuScene, false, true, true);
			}
			}
		}
	...
	protected MenuScene createMenuScene() {
		...
		// menuScene.setBackground(new SpriteBackground(new Sprite(200,100,
		// this.tmenu)));
	...
	public void mostrarPuntuacion() {
		...
		if (turnos == 1) {
			if (anoto) {
				...
				sobre.setVisible(false);
				escena.unregisterTouchArea(sobre);
				escena.registerTouchArea(pasar);
			} else {
				pasar.setVisible(false);
				sobre.setVisible(true);
				escena.unregisterTouchArea(pasar);
				escena.registerTouchArea(sobre);
			}
		}
		boton2.setVisible(false);
	...
	private void salirPuntuacion() {
		...
		if (this.finalizarJ()) {
			...
			this.cubilete.setVisible(false);
			// this.escena.detachChild(boton3);
			this.boton1.setVisible(false);
			this.boton2.setVisible(false);
			// this.escena.detachChild(txtjugador);
			this.bocina.setVisible(false);
			...
			int ganador = 0, id = 0;
			for (int i = 0; i < NJ; i++) {
				txt[i] = new ChangeableText(5, y, CachoEquipos.this.mFont, "",
						15);
			...
			ChangeableText win = new ChangeableText(5, y,
					CachoEquipos.this.mFont, "", 30);
			if (ganador == 0) {
				win.setText("Empate a 0, si que son malos...");
			} else {
	...
	public void atachild() {
		if (ntiros != 0) {
			...
			if (!Sescalera.equals("X") || p[np][0] == 10
					|| g[np][2] == 2) {
			...
			if (!Sfull.equals("X") || p[np][0] == 10 || g[np][5] == 2) {
	...
	public void mandar(int x, String y) {
		switch (x) {
		case 1:

			puntuacion1.setText(y);
			puntuacion1.setVisible(true);
			puntuacion1.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 2:
			puntuacion2.setText(y);
			puntuacion2.setVisible(true);
			puntuacion2.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 3:
			puntuacion3.setText(y);
			puntuacion3.setVisible(true);
			puntuacion3.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 4:
			puntuacion4.setText(y);
			puntuacion4.setVisible(true);
			puntuacion4.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 5:
			puntuacion5.setText(y);
			puntuacion5.setVisible(true);
			puntuacion5.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 6:
			puntuacion6.setText(y);
			puntuacion6.setVisible(true);
			puntuacion6.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 7:
			puntuacion7.setText(y);
			puntuacion7.setVisible(true);
			puntuacion7.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 8:
			puntuacion8.setText(y);
			puntuacion8.setVisible(true);
			puntuacion8.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 9:
			puntuacion9.setText(y);
			puntuacion9.setVisible(true);
			puntuacion9.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		case 10:
			puntuacion10.setText(y);
			puntuacion10.setVisible(true);
			puntuacion10.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		default:
			puntuacion11.setText(y);
			puntuacion11.setVisible(true);
			puntuacion11.setColor(0.7764f, 0.0f, 0.0f, 0.5f);
			break;
		}

	}

	public boolean finalizarJ() {
		for(int i=0;i<NJ;i++){
			if(p[i][0]<11){
				return false;
			}
...
public void mostrarAll(int x){
	...
	public void terminar(){
		if (finalizarJ()) {
			...
			dado4.setVisible(false);
			dado5.setVisible(false);
			cubilete.setVisible(false);
			// this.escena.detachChild(boton3);
			boton1.setVisible(false);
			boton2.setVisible(false);
			// this.escena.detachChild(txtjugador);
			bocina.setVisible(false);
			...
			escena.unregisterTouchArea(boton1);
			escena.unregisterTouchArea(boton2);
			escena.unregisterTouchArea(boton3);
			escena.unregisterTouchArea(bocina);
			// if(CPT(0)>CPT(1)){
			SpriteBackground fn = new SpriteBackground(new Sprite(0, 0, mBgTexture2));
			escena.setBackground(fn);
